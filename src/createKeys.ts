...
const DATA_SRC    = process.env.HIVEGUARD_DATA_DIR ? path.join(process.env.HIVEGUARD_DATA_DIR, 'src') : __dirname;
const keypairsDir = path.join(DATA_SRC, 'keypairs');
const keyInfoPath = path.join(DATA_SRC, 'keyInfo.json');
...
/** Non-interactive: generate exactly `count` fresh keypairs and save them. */
export async function generateKeypairsUI(count: number = 24): Promise<void> {
  if (!fs.existsSync(keypairsDir)) fs.mkdirSync(keypairsDir, { recursive: true });

  const wallets: Keypair[] = [];
  for (let i = 0; i < count; i++) {
    const kp = Keypair.generate();
    saveKeypairToFile(kp, i);
    wallets.push(kp);
  }
  updatePoolInfo(wallets);
}
