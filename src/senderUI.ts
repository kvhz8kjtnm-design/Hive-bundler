...
/** Non-interactive: fund all sub-wallets from payer using saved buy amounts. */
export async function fundWalletsWithTip(jitoTipSol: number): Promise<void> {
	const jitoTipAmt = jitoTipSol * LAMPORTS_PER_SOL;
	const { blockhash } = await connection.getLatestBlockhash();
	const solIxs = await generateSOLTransferForKeypairs(jitoTipAmt);
	const solTxns = await processInstructionsSOL(solIxs, blockhash);
	await sendBundle(solTxns);
}

/** Non-interactive: calculate and save buy amounts for all wallets. */
export function saveBuyAmountsUI(devSol: number, walletSol: number): {
	totalSol: number; totalPct: number; walletCount: number
} {
	const keypairs = loadKeypairs();
	const tokenDecimals = 10 ** 6;
	...
	const entries = [{ kp: wallet, sol: devSol * 1.21 }, ...keypairs.map(kp => ({ kp, sol: walletSol }))];

	for (const { kp, sol } of entries) {
		const solLamports = sol * LAMPORTS_PER_SOL;
		const e = new BN(solLamports);
		const initialVirtualSolReserves = 30 * LAMPORTS_PER_SOL + initialRealSolReserves;
		const a = new BN(initialVirtualSolReserves).mul(new BN(initialVirtualTokenReserves));
		const i = new BN(initialVirtualSolReserves).add(e);
		const l = a.div(i).add(new BN(1));
		...
		const tokensBought = tokensToBuy.toNumber();
		const percentSupply = (tokensBought / tokenTotalSupply) * 100;
		buys.push({ pubkey: kp.publicKey, solAmount: sol, tokenAmount: tokensToBuy, percentSupply });
		initialRealSolReserves    += e.toNumber();
		initialRealTokenReserves  -= tokensBought;
		initialVirtualTokenReserves -= tokensBought;
		totalTokensBought += tokensBought;
	...
	return {
		totalSol:     entries.reduce((s, e) => s + e.sol, 0),
		totalPct:     (totalTokensBought / tokenTotalSupply) * 100,
