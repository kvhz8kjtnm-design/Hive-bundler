...
/** Non-interactive version of createLUT — accepts tip instead of prompting. */
export async function createLUTWithTip(jitoTipSol: number): Promise<void> {
    const jitoTipAmt = jitoTipSol * LAMPORTS_PER_SOL;
    let poolInfo: { [key: string]: any } = {};
    if (fs.existsSync(keyInfoPath)) {
        const data = fs.readFileSync(keyInfoPath, 'utf-8');
        poolInfo = JSON.parse(data);
    ...
    createLUTixs.push(create, SystemProgram.transfer({
        fromPubkey: payer.publicKey,
        toPubkey:   getRandomTipAccount(),
        lamports:   jitoTipAmt,
    }));
    const addressesMain: PublicKey[] = [];
    createLUTixs.forEach(ixn => ixn.keys.forEach(k => addressesMain.push(k.pubkey)));
    const lookupTablesMain1 = lookupTableProvider.computeIdealLookupTablesForAddresses(addressesMain);
    const { blockhash } = await connection.getLatestBlockhash();
    const messageMain1 = new TransactionMessage({
        payerKey: payer.publicKey, recentBlockhash: blockhash, instructions: createLUTixs,
    }).compileToV0Message(lookupTablesMain1);
...
/** Non-interactive version of extendLUT — accepts tip (and optional vanity key). */
export async function extendLUTWithParams(jitoTipSol: number, vanityPK?: string): Promise<void> {
    const jitoTipAmt = jitoTipSol * LAMPORTS_PER_SOL;
    let poolInfo: { [key: string]: any } = {};
    if (fs.existsSync(keyInfoPath)) {
        const data = fs.readFileSync(keyInfoPath, 'utf-8');
    ...
    if (!poolInfo.addressLUT) throw new Error('No LUT address found — create the LUT first.');
    const bundledTxns1: VersionedTransaction[] = [];
    const accounts: PublicKey[] = [];
    const lut = new PublicKey(poolInfo.addressLUT.toString());
    const lookupTableAccount = (await connection.getAddressLookupTable(lut)).value;
    if (!lookupTableAccount) throw new Error('LUT account not found on-chain.');
    const mintKp = vanityPK ? Keypair.fromSecretKey(bs58.decode(vanityPK)) : Keypair.generate();
    console.log(`Mint: ${mintKp.publicKey.toString()}`);
    poolInfo.mint   = mintKp.publicKey.toString();
    poolInfo.mintPk = bs58.encode(mintKp.secretKey);
    fs.writeFileSync(keyInfoPath, JSON.stringify(poolInfo, null, 2));
    const mintAuthority_      = new PublicKey('TSLvdd1pWpHVjahSpsvCXUbgwsL3JAcvokwaKt1eokM');
    const MPL_               = new PublicKey('metaqbxxUerdq28cj1RbAWkYQm3ybzjb6a8bt518x1s');
    const global_            = new PublicKey('4wTV1YmiEkRvAtNtsSGPtUrqRYQMe5SKy2uB4Jjaxnjf');
    const [bondingCurve_]    = PublicKey.findProgramAddressSync([Buffer.from('bonding-curve'), mintKp.publicKey.toBytes()], PUMP_PROGRAM);
    const [metadata_]        = PublicKey.findProgramAddressSync([Buffer.from('metadata'), MPL_.toBytes(), mintKp.publicKey.toBytes()], MPL_);
    const [assocBonding_]    = PublicKey.findProgramAddressSync([bondingCurve_.toBytes(), spl.TOKEN_PROGRAM_ID.toBytes(), mintKp.publicKey.toBytes()], spl.ASSOCIATED_TOKEN_PROGRAM_ID);
    const eventAuthority_    = new PublicKey('Ce6TQqeHC9p8KetsN6JsjHK7UTZk7nasjjnr7XxXp9F1');
    const feeRecipient_      = new PublicKey('CebN5WGQ4jvEPvsVU4EoHEpgzq1VV7AbicfhtW4xC9iM');
    accounts.push(spl.ASSOCIATED_TOKEN_PROGRAM_ID, spl.TOKEN_PROGRAM_ID, MPL_, mintAuthority_, global_, PUMP_PROGRAM, metadata_, assocBonding_, bondingCurve_, eventAuthority_, SystemProgram.programId, SYSVAR_RENT_PUBKEY, mintKp.publicKey, feeRecipient_);
    const keypairs = loadKeypairs();
    for (const keypair of keypairs) {
        const ataToken = await spl.getAssociatedTokenAddress(mintKp.publicKey, keypair.publicKey);
        accounts.push(keypair.publicKey, ataToken);
    }
    const ataWall  = await spl.getAssociatedTokenAddress(mintKp.publicKey, wallet.publicKey);
    const ataPayer = await spl.getAssociatedTokenAddress(mintKp.publicKey, payer.publicKey);
    accounts.push(wallet.publicKey, payer.publicKey, ataWall, ataPayer, lut, spl.NATIVE_MINT);
    const extendLUTixs1: TransactionInstruction[] = [];
    const extendLUTixs2: TransactionInstruction[] = [];
    const extendLUTixs3: TransactionInstruction[] = [];
    const extendLUTixs4: TransactionInstruction[] = [];
    const chunks = Array.from({ length: Math.ceil(accounts.length / 30) }, (_, i) => accounts.slice(i * 30, (i + 1) * 30));
    for (let i = 0; i < chunks.length; i++) {
        const ix = AddressLookupTableProgram.extendLookupTable({ lookupTable: lut, authority: payer.publicKey, payer: payer.publicKey, addresses: chunks[i] });
        [extendLUTixs1, extendLUTixs2, extendLUTixs3, extendLUTixs4][Math.min(i, 3)].push(ix);
    }
    extendLUTixs4.push(SystemProgram.transfer({ fromPubkey: payer.publicKey, toPubkey: getRandomTipAccount(), lamports: BigInt(jitoTipAmt) }));
    const { blockhash: block1 } = await connection.getLatestBlockhash();
    const txns = await Promise.all([extendLUTixs1, extendLUTixs2, extendLUTixs3, extendLUTixs4].map(ixs => buildTxn(ixs, block1, lookupTableAccount)));
    bundledTxns1.push(...txns);
    await sendBundle(bundledTxns1);
}
